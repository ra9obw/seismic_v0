...
SDMMC_BLOCK_t *const sdmmc_block[1] = {&SDMMC_BLOCK_0};
...
SDMMC_BLOCK_STATUS_t lResult = -1;
	...
	{
		lResult = SDMMC_BLOCK_ReadBlock(sdmmc_block[0], pucBuffer, ulSectorNumber, (uint8_t)ulSectorCount);
		if(lResult == SDMMC_BLOCK_STATUS_SUCCESS)
		{
			lReturnCode = 0L;
		}
		else
...
//		}
//
		lResult = SDMMC_BLOCK_WriteBlock(sdmmc_block[0], pucBuffer, ulSectorNumber, (uint8_t)ulSectorCount);
		if(lResult == SDMMC_BLOCK_STATUS_SUCCESS)	//if( lResult != pdFALSE )
		{
			/* No errors. */
			lReturnCode = 0L;
		}
...
//			Divide by two to get the number of 512-byte sectors. */
			sdmmc_status = SDMMC_BLOCK_Ioctl(sdmmc_block[0], SDMMC_BLOCK_GET_SECTOR_SIZE, &sector_size);
			sdmmc_status = SDMMC_BLOCK_Ioctl(sdmmc_block[0], SDMMC_BLOCK_GET_SECTOR_COUNT, &sector_count);
			pxDisk->ulNumberOfSectors = sector_count;//sdmmc_block[0]->sdmmc_sd->block_count;

//			pxDisk->ulNumberOfSectors = SDMMC_BLOCK_0.
...
//		xReturn = pdPASS;
//	}
	if(SDMMC_BLOCK_Init(sdmmc_block[xDriveNumber]) == SDMMC_BLOCK_STATUS_SUCCESS)
	{
		if(SDMMC_BLOCK_Initialize(sdmmc_block[xDriveNumber]) == SDMMC_BLOCK_STATUS_SUCCESS)
		{
			 if(SDMMC_BLOCK_GetStatus(sdmmc_block[xDriveNumber]) == SDMMC_BLOCK_CARD_STATE_INITIALIZED)
			 {
				 xReturn = pdPASS;
			 }
		}
