 ...
 * main.c
 *
 *  Created on: 2017 Feb 07 10:51:43
 *  Author: pavlenko_av
 */
...
#define mainLED_TOGGLE_PERIOD			pdMS_TO_TICKS( 125 )
 ...
 */
#define TEST_FS_SPEED	0
#define VERIFY_DATA		0

#define ENA_TCPIP		1
#define ENA_ADC_TASK	1
...
#define mainDEVICE_NICK_NAME			"xmc45"
...
//static void TestTmrCallback(TimerHandle_t xTimer);
//unsigned int	testVariable = 100;
//TimerHandle_t 	xTest_Timer;
...
int freertos_main(void)
{
...
#endif//Enable TCP stack operation

#if(( ( mainCREATE_FTP_SERVER == 1 ) || ( mainCREATE_HTTP_SERVER == 1 ) ) && ENA_TCPIP)
	{
	...
	xTaskCreate( Led_Task, "Task0", 2048U, NULL, ( tskIDLE_PRIORITY ), NULL );

#if ENA_ADC_TASK
	init_data_acquisition(tskIDLE_PRIORITY);
#endif//Enable ADC+DAC+PLL control task
...
#if ENA_ADC_TASK
	init_data_acq_task(tskIDLE_PRIORITY);
#endif//Enable ADC+DAC+PLL control task
...
void Led_Task(void *pvParameters)
{
	...
	static int BUFF_NUM = 1024;
	static int N_FILE = 475;
	static int N_PORTION = 1;
	static int bufSize = 4096;

	int rd_name_ind = 0, wr_name_ind = 0;
	int ii=0, kk=0, jj=0;
	char str[] = "dt_%d.bin";
	char fil_name[80];
	...
	//Initial data fill
	for(kk=0; kk<(bufSize); kk++)
	{
		dbuff[kk] = ((wr_name_ind+kk+jj)&0xFF) ^ 0xFF;
	}
	...
	while(1)
	{
		...
		//Write N_PORTION files
//		DIGITAL_IO_SetOutputHigh(&LED_0);
		for(ii=0; ii<N_PORTION; ii++)
		{
			sprintf(fil_name, str, wr_name_ind);
			pxSourceFile = ff_fopen( fil_name, "w");
			if (pxSourceFile == NULL)
			{
				goto FAILURE;
			}
			//write multiple times
			for(jj=0;jj<BUFF_NUM;jj++)
			{
...
#endif
				/* Write a message */
				xSize = ff_fwrite( dbuff, 1, bufSize, pxSourceFile );
//				res = f_sync(&fil);
				if (xSize != bufSize )
				{
					goto FAILURE;
			...
			if(ff_fclose(pxSourceFile) != 0)
			{
				goto FAILURE;
			}

			if(++wr_name_ind == N_FILE)
			{
				wr_name_ind = 0;
			}
		}
		//Read N_PORTION files
//		DIGITAL_IO_SetOutputLow(&LED_0);
		for(ii=0; ii<N_PORTION; ii++)
		{
			sprintf(fil_name, str, rd_name_ind);
			pxSourceFile = ff_fopen( fil_name, "r");
			if (pxSourceFile == NULL)
			{
				goto FAILURE;
			}
			...
			for(jj=0;jj<BUFF_NUM;jj++)
			{
				if( ff_fseek( pxSourceFile, jj*bufSize, FF_SEEK_SET ) != 0 )
				{
					goto FAILURE;
				}
				/* Read the buffer from file */
				xSize = ff_fread( dbuff, 1, bufSize, pxSourceFile );
				if (xSize != bufSize )
				{
					goto FAILURE;
				}
...
//				DIGITAL_IO_SetOutputHigh(&LED_1);
				for(kk=0; kk<bufSize; kk++)
				{
					if( dbuff[kk] != (((rd_name_ind+kk+jj)&0xFF) ^ 0xFF) )
					{
						goto FAILURE;
					}
				}
...
//	f_close(&fil_rd);
	...
	{
		vTaskDelay(30);
//		DIGITAL_IO_ToggleOutput(&LED_1);/* Endless loop */
	}
#else
//	xTest_Timer = xTimerCreate( "test_tmr",	100, pdFALSE, ( void * ) (&testVariable), TestTmrCallback );
//
//	if( xTest_Timer != NULL )//if (erase_tmr_id != NULL)
//	{
//		/* Start the OS timer now */
//		xTimerStart( xTest_Timer, 10 );
//		do
//		{
//			if (testVariable == 0)
//			{
...
//		xTimerStop  ( xTest_Timer, 10 );
//		xTimerDelete( xTest_Timer, 10 );
//
//	}
	while(1)
	{

		vTaskDelay(mainLED_TOGGLE_PERIOD);
		DIGITAL_IO_ToggleOutput(&LED_1);/* Endless loop */
		vTaskDelay(mainLED_TOGGLE_PERIOD);
		DIGITAL_IO_ToggleOutput(&LED_1);/* Endless loop */

//		if(DIGITAL_IO_GetInput(&PUSH_BTN) == 1)
//		{
//			DIGITAL_IO_SetOutputLow(&LED_0);/* Endless loop */
//		}
//		else
//		{
//			DIGITAL_IO_SetOutputHigh(&LED_0);/* Endless loop */
//		}
	}
#endif
}
...
////	if(xTimer != 0)
//	{
//	    unsigned int *obj = (unsigned int *) pvTimerGetTimerID( xTimer );
//	    *obj = 0;
//	}
//}
/*-----------------------------------------------------------*/
...
static void prvInitialiseHeap(void)
{
	...
	extern char Heap_Bank2_Start;
	extern char Heap_Bank3_Start;

	extern char Heap_Bank1_Size;
	extern char Heap_Bank2_Size;
	extern char Heap_Bank3_Size;

	uint8_t *pheap_bank1_start_addr = ( uint8_t * )&Heap_Bank1_Start;
	size_t heap_bank1_size_var = (size_t)&Heap_Bank1_Size;

	uint8_t *pheap_bank2_start_addr = ( uint8_t * )&Heap_Bank2_Start;
	size_t heap_bank2_size_var = (size_t)&Heap_Bank2_Size;

	uint8_t *pheap_bank3_start_addr = ( uint8_t * )&Heap_Bank3_Start;
	size_t heap_bank3_size_var = (size_t)&Heap_Bank3_Size;

	//	pucHeapStart = ( uint8_t * ) ( ( heap_bank1_start_addr + 7 ) & ~0x07ul );
	//
	//	ulHeapSize = (uint32_t)heap_bank1_size_var;//((uint32_t)0x10000 - (uint32_t)0x800);//( uint32_t ) ( ( uint32_t ) pucHeapEnd - ( uint32_t ) &mainHEAP_START );
	//	ulHeapSize &= ~0x07ul;
	//	ulHeapSize -= 1024;
	...
	HeapRegion_t xHeapRegions[] = {
			...
			{ ( uint8_t *) pheap_bank3_start_addr, heap_bank3_size_var },
#endif
			{ ( uint8_t *) pheap_bank1_start_addr, heap_bank1_size_var },
			{ ( uint8_t *) pheap_bank2_start_addr, heap_bank2_size_var },
			{ NULL, 0 }
	};
